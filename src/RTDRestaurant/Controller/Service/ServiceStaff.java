...
public class ServiceStaff {
    ...
    public ModelNhanVien getStaff(int userID) throws SQLException {
        ...
        String sql = "SELECT ID_NV, TenNV, FORMAT(NgayVL,'yyyy-mm-dd') AS Ngay, SDT, Chucvu, ID_NQL FROM NhanVien WHERE ID_ND=?";
    ...
    public ArrayList<ModelPNK> MenuPNK() throws SQLException {
        ...
        String sql = "SELECT ID_NK,ID_NV,FORMAT(NgayNK,'yyyy-mm-dd') AS Ngay,Tongtien FROM PhieuNK ORDER BY ID_NK";
        PreparedStatement p = con.prepareStatement(sql);
        ResultSet r = p.executeQuery();
        while (r.next()) {
            int idNK = r.getInt(1);
    ...
    public ModelPNK getPNKbyID(int id) throws SQLException {
        ModelPNK data = null;
        String sql = "SELECT ID_NK,ID_NV,FORMAT(NgayNK,'yyyy-mm-dd') AS Ngay,Tongtien FROM PhieuNK WHERE ID_NK=?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, id);
        ResultSet r = p.executeQuery();
        while (r.next()) {
    ...
    public ArrayList<ModelPXK> MenuPXK() throws SQLException {
        ...
        String sql = "SELECT ID_XK,ID_NV,FORMAT(NgayXK,'yyyy-mm-dd') AS Ngay FROM PhieuXK ORDER BY ID_XK";
        PreparedStatement p = con.prepareStatement(sql);
        ResultSet r = p.executeQuery();
        while (r.next()) {
            int idXK = r.getInt(1);
    ...
    public ModelPXK getPXKbyID(int id) throws SQLException {
        ModelPXK data = null;
        String sql = "SELECT ID_XK,ID_NV,FORMAT(NgayXK,'yyyy-mm-dd') AS Ngay FROM PhieuXK WHERE ID_XK=?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, id);
        ResultSet r = p.executeQuery();
        while (r.next()) {
    ...
    public ArrayList<ModelKhachHang> MenuKH() throws SQLException {
        ...
        String sql = "SELECT ID_KH, TenKH, FORMAT(NgayThamGia,'yyyy-mm-dd') AS Ngay, Doanhso, Diemtichluy FROM KhachHang";
        PreparedStatement p = con.prepareStatement(sql);
        ResultSet r = p.executeQuery();
        while (r.next()) {
    ...
    // Điều chỉnh trạng thái bàn thành Đã đặt trước sau khi nhân viên xác
    // nhận
    ...
    // Cập nhật trạng thái Hóa đơn thành Đã thanh toán khi Nhân viên xác
    // nhận thanh toán
    ...
    public void updateBanTrong(int idHD) throws SQLException {
        String sql = "UPDATE Ban SET TrangThai = 'Con trong' WHERE ID_Ban = (SELECT ID_Ban FROM HoaDon WHERE ID_HoaDon = ?)";

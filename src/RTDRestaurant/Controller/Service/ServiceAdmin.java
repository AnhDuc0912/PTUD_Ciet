...
public class ServiceAdmin {
    ...
    public ArrayList<ModelNhanVien> getListNV() throws SQLException {
        ...
        String sql = "SELECT ID_NV,TenNV, FORMAT(NgayVL,'yyyy-mm-dd') as Ngay,SDT,ChucVu,ID_NQL,TinhTrang FROM NhanVien";
    ...
    public ModelNhanVien getNV(int idNV) throws SQLException {
        ...
        String sql = "SELECT ID_NV,TenNV,FORMAT(NgayVL,'yyyy-mm-dd') as Ngay,SDT,ChucVu,ID_NQL,TinhTrang FROM NhanVien WHERE ID_NV=?";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, idNV);
        ResultSet r = p.executeQuery();
        while (r.next()) {
    ...
    public void insertNV(ModelNhanVien data) throws SQLException, ParseException {
        String sql = "INSERT INTO NhanVien (ID_NV, TenNV, NgayVL, SDT, Chucvu, ID_NQL, TinhTrang) VALUES (?, ?, ?, ?, ?, ?, 'Dang lam viec')";
        PreparedStatement p = con.prepareStatement(sql);
        p.setInt(1, data.getId_NV());
        p.setString(2, data.getTenNV());

        String ngayVLString = data.getNgayVL();
        java.util.Date ngayVLUtil = new SimpleDateFormat("dd/MM/yyyy").parse(ngayVLString);
        java.sql.Date ngayVL = new java.sql.Date(ngayVLUtil.getTime());
    ...
    public ArrayList<ModelHoaDon> getListHDIn(String txt) throws SQLException {
        ...
        String sql = "SELECT ID_HoaDon, ID_KH, ID_Ban, FORMAT(NgayHD, 'yyyy-MM-dd') AS Ngay, Tienmonan, Tiengiam, Tongtien "
                +
                "FROM HoaDon";
        if (txt.equals("Tất cả")) {
            sql = "SELECT ID_HoaDon, ID_KH, ID_Ban, FORMAT(NgayHD, 'yyyy-MM-dd') AS Ngay, Tienmonan, Tiengiam, Tongtien "
                    +
                    "FROM HoaDon";
        } else if (txt.equals("Hôm nay")) {
            sql = "SELECT ID_HoaDon,ID_KH,ID_Ban,FORMAT(NgayHD,'yyyy-mm-dd')as Ngay,Tienmonan,Tiengiam,Tongtien FROM HoaDon "
                    + "WHERE DAY(NgayHD) = DAY(GETDATE())";
            ...
            sql = "SELECT ID_HoaDon, ID_KH, ID_Ban, " +
                    "FORMAT(NgayHD, 'yyyy-MM-dd') AS Ngay, Tienmonan, Tiengiam, Tongtien " +
                    "FROM HoaDon " +
    ...
    public int getRevenueHD(String filter) throws SQLException {
        ...
        String sql = "SELECT SUM(Tongtien) FROM HoaDon WHERE DAY(NgayHD) = DAY(GETDATE())";
        if (filter.equals("Hôm nay")) {
            sql = "SELECT SUM(Tongtien) FROM HoaDon WHERE DAY(NgayHD) = DAY(GETDATE())";
    ...
    public int getPreMonthRevenueHD() throws SQLException {
        ...
        String sql = "SELECT SUM(Tongtien) FROM HoaDon " +
                "WHERE MONTH(NgayHD) = MONTH(GETDATE()) - 1 AND YEAR(NgayHD) = YEAR(GETDATE())";
    ...
    public ArrayList<ModelPNK> getListPNKIn(String txt) throws SQLException {
        ...
        String sql = "SELECT ID_NK, ID_NV, FORMAT(NgayNK, 'yyyy-MM-dd') AS Ngay, Tongtien " +
                "FROM PhieuNK " +
                "ORDER BY ID_NK";
        if (txt.equals("Tất cả")) {
            sql = "SELECT ID_NK, ID_NV, FORMAT(NgayNK, 'yyyy-MM-dd') AS Ngay, Tongtien " +
                    "FROM PhieuNK " +
                    "ORDER BY ID_NK";
        } else if (txt.equals("Hôm nay")) {
            sql = "SELECT ID_NK, ID_NV, FORMAT(NgayNK, 'yyyy-MM-dd') AS Ngay, Tongtien " +
                    "FROM PhieuNK " +
                    "WHERE CONVERT(DATE, NgayNK, 105) = CONVERT(DATE, GETDATE(), 105) " +
                    "ORDER BY ID_NK";
        } else if (txt.equals("Tháng này")) {
            sql = "SELECT ID_NK, ID_NV, FORMAT(NgayNK, 'yyyy-MM-dd') AS Ngay, Tongtien " +
                    "FROM PhieuNK " +
                    "WHERE DATEPART(YEAR, NgayNK) = DATEPART(YEAR, GETDATE()) " +
                    "AND DATEPART(MONTH, NgayNK) = DATEPART(MONTH, GETDATE()) " +
                    "ORDER BY ID_NK";
        } else if (txt.equals("Năm này")) {
            sql = "SELECT ID_NK, ID_NV, FORMAT(NgayNK, 'yyyy-MM-dd') AS Ngay, Tongtien " +
                    "FROM PhieuNK " +
    ...
    public int getCostNK(String filter) throws SQLException {
        ...
        String sql = "SELECT SUM(Tongtien) FROM PhieuNK WHERE DAY(NgayNK) = DAY(GETDATE())";
        if (filter.equals("Hôm nay")) {
            sql = "SELECT SUM(Tongtien) FROM PhieuNK WHERE DAY(NgayNK) = DAY(GETDATE())";
        } else if (filter.equals("Tháng này")) {
    ...
    public int getPreMonthCostNK() throws SQLException {
        int Pre_Cost = 0;
        String sql = "SELECT SUM(Tongtien) AS TotalAmount " +
                "FROM PhieuNK " +
                "WHERE MONTH(NgayNK) = MONTH(GETDATE()) - 1 " +
    ...
    public ArrayList<ModelChart> getRevenueCostProfit_byMonth() throws SQLException {
        ...
        String sql_Revenue = "SELECT MONTH(NgayHD) AS Thang, SUM(TONGTIEN) AS TongTien " +
                "FROM HoaDon " +
                "WHERE YEAR(NgayHD) = YEAR(GETDATE()) " +
                "GROUP BY MONTH(NgayHD) " +
                "ORDER BY Thang";
        String sql_Cost = "SELECT MONTH(NgayNK) AS Thang, SUM(TONGTIEN) AS TongTien " +
                "FROM PhieuNK " +
                "WHERE YEAR(NgayNK) = YEAR(GETDATE()) " +
                "GROUP BY MONTH(NgayNK) " +
                "ORDER BY Thang";

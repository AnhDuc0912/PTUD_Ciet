...
public class ServiceCustomer {
    ...
    public void InsertHoaDon(ModelBan table, ModelKhachHang customer) throws SQLException {
        ...
        p.setDate(4, new java.sql.Date(new Date().getTime()));
        p_ID.close();
        r_id.close();
        p.execute();
        p.close();
    ...
    public ArrayList<ModelHoaDon> getListHDOrder(int ID_KH, String order) throws SQLException {
        ...
        switch (order) {
            case "Tất cả":
                sql = "SELECT ID_HoaDon,ID_KH,ID_Ban,FORMAT(NgayHD, 'yyyy-mm-dd') AS Ngay,TienMonAn,Code_Voucher,TienGiam,Tongtien,Trangthai FROM HoaDon "
                        + "WHERE ID_KH=? ORDER BY ID_HoaDon";
                break;
